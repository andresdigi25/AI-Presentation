...
class Ray:
    def __init__(self, angle, player, map):
        ...
        # Added attributes for hit detection
    ...
    def cast(self):
        ...
        while (nextHorizontalX <= WINDOW_WIDTH and nextHorizontalX >= 0 and nextHorizontalY <= WINDOW_HEIGHT and nextHorizontalY >= 0):
            ...
            if (
                0 <= grid_y < len(self.map.grid)
                and 0 <= grid_x < len(self.map.grid[0])
            ):
                self.wall_type = self.map.grid[grid_y][grid_x]
            else:
                self.wall_type = 1  # Default wall type if out of bounds
        ...
        if horizontal_distance < vertical_distance:
            ...
            self.color = 160
            # When you detect a horizontal wall hit:
            self.hit_vertical = False
            self.hit_x = horizontal_hit_x
            self.hit_y = horizontal_hit_y
        else:
            self.wall_hit_x = vertical_hit_x
            self.wall_hit_y = vertical_hit_y
            self.distance = vertical_distance
            self.color = 255
            # When you detect a vertical wall hit:
            self.hit_vertical = True
            self.hit_x = vertical_hit_x
            self.hit_y = vertical_hit_y
