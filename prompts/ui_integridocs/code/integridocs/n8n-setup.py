...
N8N_PASSWORD = os.getenv('N8N_PASSWORD', 'Testing321')
...
with open('/tmp/workflows/ia_docs.json', 'r') as file:
    data = json.load(file)

    for node in data['nodes']:
        if node.get('credentials', {}).get('openAiApi', {}).get('name') == 'OpenAi account':
